...
RSpec.describe Rumale::MetricLearning::MLKR do
  describe 'metric learning' do
    ...
    it 'dumps and restores itself using Marshal module.', :aggregate_failures do
      ...
      expect(copied.prototypes).to eq(transformer.prototypes)
      expect(copied.values).to eq(transformer.values)
  ...
  describe 'regression' do
    ...
    let(:x) { dataset[0] }
    let(:y) { dataset[0][true, 0] + dataset[0][true, 1]**2 }
    let(:n_features) { x.shape[1] }
    let(:n_samples) { x.shape[0] }
    let(:estimator) { described_class.new(random_seed: 1).fit(x, y) }
